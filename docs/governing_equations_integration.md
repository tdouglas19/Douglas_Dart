...
| `ramjet.py` `ideal_inlet_shock_recovery` | **Changed** (docs/ramjet_enginesim_comparison.md "Difference 1"). pi_d is Mach-dependent: 1.0 (lossless) at/below Mach 1. Above Mach 1 it previously used the normal-shock total-pressure ratio, a single-normal-shock model this doc had already flagged as needing replacement with a multi-oblique-shock MIL-E-5008B correlation before extending past ~Mach 1.2; it now uses that correlation directly (`1.0 - 0.075*(M-1)^1.35`), cross-checked against NASA EngineSim's own default inlet-recovery option. | sec. 2.2 |
...
| File | Change | Source |
|---|---|---|
| `ramjet.py` `_solve_ramjet_cycle` fuel-air ratio | **Changed** (2026-08-10 session, `RamjetConfig.target_equivalence_ratio`). Same energy balance `f = cp*(Tt3-Tt2) / (eta_b*h_PR - cp*Tt3)`, matches the reference exactly, but now rearranged to solve for Tt3 (combustor exit temperature, a computed output) given `f` (now the input, `f = target_equivalence_ratio / stoichiometric_air_fuel_ratio`) instead of the reverse -- mirrors `pulsejet.py`'s own equivalence-ratio-driven fuel metering rather than backing `f` out of a fixed target temperature. | sec. 2.3 |
...
- ~~**Side-inlet ram-pressure credit fraction** (sec. 2.2)~~ -- **resolved 2026-08-08**:
  `SIDE_INLET_RAM_PRESSURE_CREDIT_FRACTION` (the unsourced flat 0.15) replaced by
  `pulsejet.py`'s `side_inlet_ram_recovery_ratio`, a real correlation (Hall & Frank,
  NACA RM A8I29) coupled to the fluid-inertance inlet model's own captured mass flow.
  See `docs/design_convergence.md` "Side-inlet ram recovery."
