...
| Pulsejet/ramjet switchable-mode transition Mach threshold, self-sustaining side (`minimum_self_sustaining_mach`) | **Provisional assumption**, explicitly flagged as unsourced | `config.py RamjetConfig` docstring: no patent in the switchable-engine lineage gives a quantitative transition law. |
| Pulsejet/ramjet switchable-mode transition Mach threshold, lightoff side (`minimum_lightoff_test_mach`) | **Derived, not an assumption** (changed 2026-08-10) | No longer YAML-authored. `config.py`'s `load_reference_case` computes it via `ramjet.py`'s `derive_lightoff_mach`: the Mach at which this specific engine's own net thrust first turns positive and stays positive. See `RamjetConfig`'s docstring and `docs/design_convergence.md`. |
...
| Pulsejet/ramjet installed-efficiency recovery factors (0.99 / 0.92), selector discharge coefficient (0.78), combustion efficiency (0.58), pulsejet/ramjet target equivalence ratios (1.00 / 0.60, same field name, two different config sections and values -- see `RamjetConfig`/`PulsejetConfig` docstrings), burn duration/period, combustor pressure loss | **Provisional assumption** | `docs/assumptions.md` "Propulsion-model assumptions" table -- every row already carries a "closure path," which is exactly what a provisional-assumption entry needs. Ramjet combustor exit temperature (T4) is no longer in this list -- since the 2026-08-10 combustor-equation reversal it is a **Calculated output** of `ramjet.py`'s `_solve_ramjet_cycle`, not a configured assumption. |
...
| Robustness scenario multipliers (conservative/adverse thrust, drag, mass, recovery factors) | **Provisional assumption**, explicitly non-probabilistic; **partially literature-benchmarked 2026-08-07, still not sourced** | `docs/assumptions.md`: "engineering screens, not probability statements." `docs/design_convergence.md`'s "Grounding the adverse-scenario multipliers" section benchmarks the mass-growth term against published AIAA/SAWE mass-growth-allowance conventions (finds the current +2.90 kg is *below*, not above, the commonly-cited ~15% conceptual-design figure -- i.e. arguably under-conservative) and the drag term against CFD/semi-empirical uncertainty literature (directionally plausible, no precise match found). No comparable literature benchmark was found for the thrust term at all -- it remains as unsourced as before. None of the three values were changed based on this research; see that section for the full citations and reasoning, and why the change was not made unilaterally. |
...
| `SelectorConfig.inlet_type: side` (all three active configs) | **User direction**, label sourced correlation for the physics it triggers | Set 2026-08-07 on user instruction about the real vehicle's intake geometry. Changes real model behavior vs. `"straight"`: now applies `side_inlet_ram_recovery_ratio` (Hall & Frank, NACA RM A8I29, replacing the prior unsourced flat 0.15 credit as of 2026-08-08 -- see `docs/design_convergence.md` "Side-inlet ram recovery"). |
| Pulsejet chamber-wall mass term (`mass_model.py`, added 2026-08-07) | **Provisional assumption**, explicitly flagged as unsourced | Thin-shell estimate (stainless-steel density, a real material property; the choice of *this* material/thickness for *this* chamber is the provisional part) -- see `mass_model.py`'s "Pulsejet chamber-wall mass" docstring section. Added on top of, not netted against, the existing `shared_engine_body_combustor_nozzle` budget line, which has no internal chamber-vs-nozzle breakdown to split from. |
