...
| `SelectorConfig.inlet_type: side` (all three active configs) | **User direction**, not sourced or validated | Set 2026-08-07 on user instruction about the real vehicle's intake geometry. Changes real model behavior (`SIDE_INLET_RAM_PRESSURE_CREDIT_FRACTION` damps ram-pressure credit vs. `"straight"`), not just a label. |
