...
# medium_model reads its OWN override (falling back to the simple_model
# name so a frozen-V2 parity flight can be set up with one variable).
# NOTE: step 1 of the fidelity ladder replaces this flat placeholder with
# a real drag build-up -- see medium_model/__init__.py. Until then this is
# here so the copy can be proven identical to its ancestor.
CD0_FRONTAL = float(_os.environ.get(
    "MEDIUM_MODEL_CD0_FRONTAL",
